...
class RunnerBase:
    ...
    def run_one_item(self, qitem, worker):
        # run the prediction
        processed_results = []
        try:
            results = requests.post("http://" + worker + ":5000/predict", json={"img": qitem.img.tolist()}).json()
            results = [np.array(i) for i in results["res"]]
    ...
    def enqueue(self, query_samples, worker):
        idx = [q.index for q in query_samples]
        query_id = [q.id for q in query_samples]  
        if len(query_samples) < self.max_batchsize:
            data, label = self.ds.get_samples(idx)
            self.run_one_item(Item(query_id, idx, data, label), worker)
        else:
            bs = self.max_batchsize
            for i in range(0, len(idx), bs):
                data, label = self.ds.get_samples(idx[i:i+bs])
                self.run_one_item(Item(query_id[i:i+bs], idx[i:i+bs], data, label), worker)
...
def add_results(final_results, name, result_dict, result_list, took, show_accuracy=True):
    ...
    # to stdout
    log.info("--RESULTS:{} qps={:.2f}, mean={:.4f}, time={:.3f}{}, queries={}, tiles={}".format(
...
def main():
    ...
    if _type is not None and _type == "worker":

        # load model to backend
        model = backend.load(args.model, inputs=args.inputs, outputs=args.outputs)
        print("Worker Option Enabled")
        ...
        def predict():
            ...
            img = np.array(item['img']).astype(np.float32)
            results = model.predict({model.inputs[0]: img})
            return {"res": [i.tolist() for i in results]}
        app.run("0.0.0.0")
    else:
        print("Server Option Enabled")
        timeout = 0
        workers = os.getenv("workers").split(',')
        for worker in workers:
            server_up = False
            while (not server_up and timeout < 120):
                try:
                    server_up = True
                    r = requests.post("http://" + worker + ":5000/predict")
                    r.raise_for_status()  # Raises a HTTPError if the status is 4xx, 5xxx
                except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
                    log.error("The " + worker + " is still down")
                    timeout += 1
                    time.sleep(1)
                    server_up = False
    ...
    runner = runner_map[scenario](ds, args.threads, post_proc=post_proc, max_batchsize=args.max_batchsize)

    def issue_queries(query_samples):
        workers = os.getenv("workers").split(',')
        threads = range(int(os.getenv("worker_threads")) if "worker_threads" in os.environ and os.getenv("worker_threads").isdigit() else 1)
        for thread in threads:
            for worker in workers:
                runner.enqueue(query_samples, worker)
    ...
    add_results(final_results, "{}".format(scenario),
                result_dict, last_timeing, time.time() - ds.last_loaded, True)
