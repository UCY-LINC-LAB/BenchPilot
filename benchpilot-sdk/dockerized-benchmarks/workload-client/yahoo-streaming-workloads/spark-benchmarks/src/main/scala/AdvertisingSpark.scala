...
object KafkaRedisAdvertisingStream {
  def main(args: Array[String]) {
    ...
    val partitioning = commonConfig.get("spark.partitions") match {
    ...
    val messages_repartitioned = if (partitioning > 1) messages.repartition(partitioning) else messages
    ...
    val kafkaRawData = messages_repartitioned.map(record => (record.value().toString))
