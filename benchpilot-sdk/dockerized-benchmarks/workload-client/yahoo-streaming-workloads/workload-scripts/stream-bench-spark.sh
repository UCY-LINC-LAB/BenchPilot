...
SPARK_EXECUTOR_MEMORY=$(grep 'spark.executor.memory' $CONF_FILE | sed 's/^.*: //');
SPARK_EXECUTOR_CORES=$(grep 'spark.executor.cores' $CONF_FILE | sed 's/^.*: //');
...
start_if_needed() {
  ...
  local sleep_time="$1"
  shift
  local PID=`ps -aef | grep "$match" | grep -v grep | awk '{print $2}'`
...
stop_if_needed() {
  ...
  then
    kill -9 "$PID"
    sleep 1
    local CHECK_AGAIN=`ps -aef | grep "$match" | grep -v grep | awk '{print $2}'`
    if [[ "$CHECK_AGAIN" -ne "" ]];
    then
      kill -9 "$CHECK_AGAIN"
    fi
...
run() {
  ...
  then
    ...
    echo "START_PROCESSING: run the spark test processing"
    echo "STOP_PROCESSING: kill the spark test processing"
    echo
    echo "START_TEST: run spark test (assumes SETUP is done)"
    echo
